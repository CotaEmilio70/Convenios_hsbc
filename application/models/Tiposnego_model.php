...
class Tiposnego_model extends CI_Model {
    function __construct(){
        parent::__construct();
    }

    function Create($Tipo){
        return $this->db->insert('tipos_negociacion',$Tipo);
    }

    function Update($Tipo){
        $this->db->where('id', $Tipo['id']);
        return $this->db->update('tipos_negociacion', $Tipo);
    }

    function GetByClave($id){
        $this->db->select('*');
        $this->db->from('tipos_negociacion');
        $this->db->where('id', $id);
    ...
    function SearchByid($id){
        ...
        $this->db->like('id', $id);
        $this->db->limit(10);
        $Query = $this->db->get('tipos_negociacion');
        if($Query->num_rows() > 0){
